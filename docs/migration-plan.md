...
Manifests: namespaces `kolchurin-prod`/`kolchurin-preview`, Deployment pulling `ghcr.io/<repo>/frontend`, Service, Ingress for `kolchurin.dev`, GHCR pull secret. Previews via GH Actions `kubectl apply`/`delete` on PR open/close (reuse slug logic from app repo's `deployment/preview/deploy.sh`). Repoint `kolchurin.dev` DNS → ovh-s1. Decommission Dokploy (it died with vultr — verify nothing references it).
...
### Phase 9 — Monitoring: kube-prometheus-stack + Uptime Kuma
